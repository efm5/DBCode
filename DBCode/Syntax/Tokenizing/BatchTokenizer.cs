namespace DBCode.Syntax.Tokenizing {
   internal sealed class BatchTokenizer : ITokenizer {
      private readonly List<ITokenReader> mReaders;

      internal BatchTokenizer(ILanguageDefinition pDefinition) {
         mReaders = [
            new WhitespaceReader(),
            new BatchCommentReader(),
            new PercentVariableReader(),
            new StringReader(),
            new NumberReader(),
