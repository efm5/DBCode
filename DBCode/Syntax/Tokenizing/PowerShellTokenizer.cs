namespace DBCode.Syntax.Tokenizing {
   internal sealed class PowerShellTokenizer : ITokenizer {
      private readonly List<ITokenReader> mReaders;

      internal PowerShellTokenizer(ILanguageDefinition pDefinition) {
         mReaders = [
            new WhitespaceReader(),
            new PythonCommentReader(),
            new DollarVariableReader(),
            new StringReader(),
            new NumberReader(),
      ...
      public LanguageKind Language {
         get { return LanguageKind.PowerShell; }
      }
